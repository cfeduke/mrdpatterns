...
public class QuestionAnswerBuilder extends Configured implements Tool {
  ...
  public static abstract class HierarchyMapper extends Mapper<Object, Text, IntWritable, Text> {
    ...
    public void map(Object key, Text value, Context context) throws IOException, InterruptedException {
      Element post = null;
      try {
        post = HierarchicalXmlFragment.getXmlElementFromString(value.toString());
      } catch (Exception e) {
        e.printStackTrace();
      ...
      int postType = getIntegerAttributeOrFail(post, "PostTypeId", context, ExceptionCounters.INVALID_POST_TYPE_ID);
      if (postType == 0)
        return;
      int id;
      if (postType == 1) {
        id = getIntegerAttributeOrFail(post, "Id", context, ExceptionCounters.INVALID_ID);
        if (id == 0)
          return;
        outValue.set("Q" + value.toString());
      } else {
        id = getIntegerAttributeOrFail(post, "ParentId", context, ExceptionCounters.INVALID_PARENT_ID);
        if (id == 0)
          return;
  ...
  private static int getIntegerAttributeOrFail(Element element, String attributeName,
      @SuppressWarnings("rawtypes") Mapper.Context context, ExceptionCounters counter) {
  ...
  public static class QuestionAnswerReducer extends Reducer<IntWritable, Text, Text, NullWritable> {
    ...
    public void reduce(IntWritable key, Iterable<Text> values, Context context) throws IOException,
      ...
      for (Text t : values) {
        ...
        if (pcd.getType() == ReduceSideDataDiscriminator.Type.QUESTION) {
          ...
          context.getCounter(DataTypeCounters.TOTAL_QUESTIONS).increment(1);
        } else {
          answers.add(pcd.getData());
          context.getCounter(DataTypeCounters.TOTAL_ANSWERS).increment(1);
        }
      }
